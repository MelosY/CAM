...
def create_decoder(decoder_name, num_classes, max_seq_len):
    if decoder_name == 'tf_decoder':
        decoder = TFDecoder(
            num_classes=num_classes,
            max_seq_len=max_seq_len,)
    elif decoder_name == 'small_tf_decoder':
        decoder = TFDecoder(
            n_layers=2,
            d_embedding=512,
            n_head=8,
            ...
            num_classes=num_classes,
            max_seq_len=max_seq_len,)
    elif decoder_name == 'tiny_tf_decoder':
        decoder = TFDecoder(
            n_layers=2,
            d_embedding=384,
            n_head=8,
            ...
            num_classes=num_classes,
            max_seq_len=max_seq_len,)
    elif decoder_name == 'minute_tf_decoder':
        decoder = TFDecoder(
            n_layers=1,
            d_embedding=384,
            n_head=8,
    ...
    elif decoder_name == 'corres_tiny_tf_decoder':
        decoder = TFDecoder(
            n_layers=6,
            d_embedding=192,
            n_head=8,
            ...
            num_classes=num_classes,
            max_seq_len=max_seq_len,)
    elif decoder_name == 'corres_small_tf_decoder':
        decoder = TFDecoder(
            n_layers=6,
            d_embedding=384,
            n_head=8,
            ...
            num_classes=num_classes,
            max_seq_len=max_seq_len,)
    elif decoder_name == 'corres_base_tf_decoder':
        decoder = TFDecoder(
            n_layers=6,
            d_embedding=512,
            n_head=8,
...
class BaseDecoder(nn.Module):
    ...
    def forward(self,
        ...
        if train_mode:
            return self.forward_train(feat, out_enc, targets, tgt_lens, img_metas, cls_query_attn_maps, trg_word_emb, start_query)
        
        if beam_width > 0:
            return self.beam_search(feat, out_enc, img_metas, cls_query_attn_maps, trg_word_emb, beam_width, start_query)
        
        return self.forward_test(feat, out_enc, img_metas, cls_query_attn_maps, trg_word_emb, start_query)
...
class TFDecoder(BaseDecoder):
    ...
    def _attention(self, trg_seq, tgt_lens, src, src_mask=None, cls_query_attn_maps=None, trg_word_emb=None, start_query=None):
        if trg_word_emb is not None:
            trg_embedding = trg_word_emb(trg_seq)
        else:
            trg_embedding = self.trg_word_emb(trg_seq)
        # TODO: debug
        trg_pos_encoded = self.position_enc(trg_embedding, start_query)
    ...
    def forward_train(self, feat, out_enc, targets, tgt_lens, img_metas, cls_query_attn_maps, trg_word_emb, start_query):
        r"""
        Args:
            feat (None): Unused.
        ...
        return outputs, attn_maps, attn_output

    def forward_test(self, feat, out_enc, img_metas, cls_query_attn_maps, trg_word_emb, start_query):
        src_mask = None
        ...
        outputs, attn_maps_list, attn_output_list = [], [], []
        for step in range(0, self.max_seq_len):
            ...
            attn_output_list.append(decoder_output[:, step, :])
    ...
    def beam_search(self, feat, out_enc, img_metas, cls_query_attn_maps, trg_word_emb, beam_width, start_query=None, eos=68):
        ...
        p = torch.cat(p, -1)[:,0,:]
        #print(p.shape)
        return p, torch.ones_like(p),None
