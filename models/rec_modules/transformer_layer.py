...
class TransformerCrossEncoderLayer(nn.Module):
    ...
    def forward(self, binary, x, mask=None):
        residual = binary
        x = self.norm1(x)
        x,attn = self.attn(binary, x, x, mask,return_attn_map=True)
        x = residual + x
        residual = x
        x = self.norm2(x)
        x = residual + self.mlp(x)
...
class PositionalEncoding(nn.Module):
    ...
    def forward(self, x, start_query=None):
        if start_query is None:
            self.device = x.device
            return x + self.position_table[:, :x.size(1)].clone().detach()
        else:
            self.device = x.device
            return torch.cat([start_query, x[:, 1:, :]], dim=1) + self.position_table[:, :x.size(1)].clone().detach()
