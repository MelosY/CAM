...
def get_single_str_list(pred_ids, dataset=None, norm_results=True):
  end_label = dataset.class_to_idx['EOS']
  unknown_label = dataset.class_to_idx['UNKNOWN']
  pad_label = dataset.class_to_idx['PADDING']
  num_samples, max_len_labels = pred_ids.size()
  pred_ids = to_numpy(pred_ids)
  ...
  for i in range(num_samples):
    ...
    for j in range(max_len_labels):
      if pred_ids[i, j] != end_label:
        if pred_ids[i, j] != unknown_label:# and pred_ids[i, j] != pad_label:
          pred_list_i.append(dataset.idx_to_class[pred_ids[i, j]])
      else:
        break
...
def get_str_list(output, target, dataset=None, norm_results=True):
  # label_seq
  assert output.dim() == 2 and target.dim() == 2

  pred_list = get_single_str_list(output, dataset=dataset, norm_results=norm_results)
  targ_list = get_single_str_list(target, dataset=dataset, norm_results=norm_results)
...
def _lexicon_search(lexicon, word):
  ...
  edit_distances = np.asarray(edit_distances, dtype=np.int32)
...
def RecPostProcess(output, target, score, dataset=None):
  pred_list, targ_list = get_str_list(output, target, dataset, norm_results=False)
...
def correctness_ed(pred_str, targ_str, max_str_len):
  ed_gt = np.zeros(max_str_len)
  blocks = Levenshtein.matching_blocks(Levenshtein.editops(pred_str, targ_str), pred_str, targ_str)
  for block in blocks[:-1]:
    pred_idx, targ_idx, block_len = block
    ed_gt[pred_idx:pred_idx+block_len].fill(1)
  return ed_gt