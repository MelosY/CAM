...
import models.rec_model as recmodel 
...
def main(args):
    ...
    # if run_on_remote:
    #     init_for_yundao(args)

    utils.init_distributed_mode(args)
    # FIXME: Interesting, `--eval` can not be parsed in the bash script.
    args.eval = args.evaluation
    
    # setting train position
    print(args)
    device = torch.device(args.device)

    timestamp = int(round(time.time()*1000))
    timestamp = time.strftime('%Y%m%d%H%M%S', time.localtime(timestamp/1000))
    sys.stdout = Logger(os.path.join(args.output_dir, 'screen_{0}.txt'.format(timestamp)))
    ...
    ## process the given strides
    temp = args.strides.split('__')
    strides = []
    for stride in temp:
        stride = stride.split('_')
        strides.append((int(stride[0]), int(stride[1])))
    model = recmodel.__dict__[args.model](
        num_classes=args.nb_classes,
        drop_path_rate=args.drop_path,
        head_init_scale=args.head_init_scale,
        decoder_name=args.decoder_type,
    ...
    if args.finetune:
        ...
        checkpoint_model = remap_checkpoint_keys(checkpoint_model, strides)
    ...
    if  args.eval:
        ...
        evaluate_Res.append(int(test_stats['acc']*10000)/100)
        # test other datasets
        if len(args.other_test_data_folders) > 0:
            eval_data_dir = os.path.dirname(args.eval_data_path.rstrip('/'))
    ...
    # override the warmup_epoch if warmup_step >0
    if args.warmup_steps > 0:
        args.warmup_epochs = args.warmup_steps / len(data_loader_train)
        ...
        if data_loader_val is not None:
            ...
            if (epoch + 1) % eval_freq == 0 or epoch + 1 == args.epochs:
                ...
                if args.model_ema and args.model_ema_eval:
                    ...
                    print(f"Accuracy of the model EMA on {len(dataset_val)} test images: {test_stats_ema['acc']:.4f}%")
    ...
    # after training, evaluate all datasets
    evaluate_Res = []
    test_stats = evaluate(data_loader_val, model, device, args=args)
    print(f"Accuracy of the network on the {len(dataset_val)} test images: {test_stats['acc']:.4f}%")
    evaluate_Res.append(int(test_stats['acc']*10000)/100)
...
if __name__ == '__main__':
    ...
    # We usually tend to save models and logs to the current dir, which is mounted on a ssd.
    # It is too expansive and unnecessary. So we make a link between the current output dir to a HDD folder.
    if args.output_dir and args.real_output_dir:
      utils.make_symlink_if_not_exists(args.real_output_dir, args.output_dir)
